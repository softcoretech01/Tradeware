...
import {
  ...
  LocalShipping as InwardIcon, Save as SaveIcon
} from '@mui/icons-material';
import { addStockInward, updateStockInward, deleteStockInward } from '../../store/stockInwardSlice';
...
const TYPE_COLORS = {
  'Purchase Receipt': { bg: '#E3F2FD', color: '#0D47A1' },
  'Import Receipt': { bg: '#F3E5F5', color: '#4A148C' },
  'Stock Transfer': { bg: '#E8EAF6', color: '#1A237E' },
  'Opening Stock': { bg: '#FFF3E0', color: '#E65100' },
  'Sales Return': { bg: '#FCE7F3', color: '#880E4F' },
};

const INWARD_TYPES = ['Purchase Receipt', 'Import Receipt', 'Stock Transfer', 'Opening Stock', 'Sales Return'];
...
const StockInward = () => {
  ...
  const transactions = useSelector(state => state.stockInward.transactions);
  ...
  const filtered = useMemo(() => {
    return transactions.filter(row => {
      if (typeFilter !== 'All' && row.inwardType !== typeFilter) return false;
  ...
  const handleDeleteConfirm = () => {
    if (deleteConfirm) {
      dispatch(deleteStockInward(deleteConfirm));
      showSnack('Stock Inward transaction deleted successfully', 'info');
  ...
  const handleFormSave = (e) => {
    ...
    if (selectedItem) {
      dispatch(updateStockInward({ oldTransaction: selectedItem, newTransaction: payload }));
      showSnack('Stock Inward transaction updated successfully');
    } else {
      dispatch(addStockInward(payload));
      showSnack('Stock Inward transaction created successfully');
    }
    setDialogOpen(false);
  };
  ...
  const handleExportExcel = useCallback(() => {
    import('xlsx').then(XLSX => {
      const exportData = filtered.map(r => ({
        'Inward ID': r.id, 'Date': r.date, 'Inward Type': r.inwardType,
        ...
        'Supplier': r.supplierName || '—', 'PO Reference': r.poReference || '—',
        'Status': r.status, 'Created By': r.createdBy,
      }));
      const ws = XLSX.utils.json_to_sheet(exportData);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Stock Inwards');
      XLSX.writeFile(wb, `Stock_Inward_${dayjs().format('YYYYMMDD_HHmm')}.xlsx`);
  ...
  const handleExportPDF = useCallback(() => {
    import('jspdf').then(jsPDFModule => {
      import('jspdf-autotable').then(() => {
        ...
        doc.text('Stock Inward Register', 14, 15);
        doc.setFontSize(9);
        doc.setTextColor(100);
        doc.text(`Generated: ${dayjs().format('DD/MM/YYYY HH:mm')}`, 14, 22);

        const head = [['Inward ID', 'Date', 'Type', 'Ref No.', 'Item Code', 'Item Name', 'Warehouse', 'Batch', 'Qty', 'Cost (₹)', 'Total (₹)', 'Status']];
        const body = filtered.map(r => [
          r.id, r.date, r.inwardType, r.refNo, r.itemCode, r.itemName,
          r.warehouse, r.batchNo || '—', r.quantity.toLocaleString(),
          r.costPrice.toFixed(2), r.totalValue.toFixed(2), r.status
        ]);
        ...
        doc.save(`Stock_Inward_${dayjs().format('YYYYMMDD_HHmm')}.pdf`);
  ...
  const columns = useMemo(() => [
    {
      field: 'id', headerName: 'Inward ID', width: 110,
      renderCell: (p) => (
    ...
    {
      field: 'inwardType', headerName: 'Inward Type', width: 140,
      renderCell: (p) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, height: '100%', p: 0 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.3 }}>
            Manage and record inward stock transactions such as Purchase Receipts, Imports, Transfers, and Returns
          </Typography>
        </Box>
      ...
      <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid #E0E0E0', display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
        <TextField
          size="small" placeholder="Search Inward ID, item code, ref no, batch…" value={search}
        ...
        <FormControl size="small" sx={{ minWidth: 180 }}>
          ...
          <Select value={typeFilter} label="Inward Type" onChange={e => setTypeFilter(e.target.value)} sx={{ borderRadius: 2 }}>
            <MenuItem value="All">All Inward Types</MenuItem>
            {INWARD_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
          </Select>
        </FormControl>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth
        ...
        <DialogTitle sx={{ fontWeight: 700, color: BLUE.dark, pb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
          <InwardIcon sx={{ color: BLUE.main }} />
          {selectedItem ? 'Edit Stock Inward Record' : 'New Stock Inward Entry'}
        </DialogTitle>
        <Divider />
          ...
          <DialogContent sx={{ py: 3 }}>
            <Grid container spacing={2.5}>
              ...
              <Grid item xs={12} sm={3}>
                <TextField label="Inward ID" size="small" fullWidth value={formData.id} disabled />
              </Grid>
              <Grid item xs={12} sm={3}>
              ...
              <Grid item xs={12} sm={3}>
                <FormControl size="small" fullWidth>
                  <InputLabel>Inward Type</InputLabel>
                  <Select value={formData.inwardType} label="Inward Type"
                    onChange={e => handleFormChange('inwardType', e.target.value)}>
                    {INWARD_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
                  </Select>
                </FormControl>
              </Grid>
              <Grid item xs={12} sm={3}>
                <TextField label="Reference No." size="small" fullWidth required placeholder="e.g. PR-2025-001"
              ...
              <Grid item xs={12} sm={4}>
                <TextField label="Supplier Name" size="small" fullWidth placeholder="e.g. Sanchin Industries"
                  value={formData.supplierName} onChange={e => handleFormChange('supplierName', e.target.value)} />
              </Grid>
              <Grid item xs={12} sm={4}>
                <TextField label="PO Reference" size="small" fullWidth placeholder="e.g. PO-2025-0042"
                  value={formData.poReference} onChange={e => handleFormChange('poReference', e.target.value)} />
              </Grid>
              <Grid item xs={12} sm={4}>
                <FormControl size="small" fullWidth>
                  <InputLabel>Status</InputLabel>
      ...
      <Dialog open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} maxWidth="xs" fullWidth
        ...
        <DialogTitle sx={{ fontWeight: 700, color: '#C62828', display: 'flex', alignItems: 'center', gap: 1 }}>
          <DeleteIcon /> Delete Inward Record
        </DialogTitle>
        <DialogContent>
          {deleteConfirm && (
            <Typography>
              Are you sure you want to delete inward entry <strong>{deleteConfirm.id}</strong> ({deleteConfirm.itemName})?
              This will subtract <strong>{deleteConfirm.quantity}</strong> units from the stock overview.
            </Typography>
          )}
        </DialogContent>
