...
const InventoryMaster = () => {
  ...
  const columns = useMemo(() => [
    ...
    {
      field: 'reservedStock', headerName: 'Reserved', width: 90, type: 'number', align: 'right', headerAlign: 'right',
      renderCell: (p) => {
        const val = p.value ?? 0;
        return (
          <Typography sx={{ fontSize: 13, fontWeight: 600, color: val > 0 ? BLUE.main : 'text.secondary' }}>
    ...
    {
      field: 'expiryDate', headerName: 'Expiry Date', width: 110, align: 'center', headerAlign: 'center',
      renderCell: (p) => {
        if (!p.value) return <Typography sx={{ fontSize: 12, color: 'text.disabled' }}>N/A</Typography>;
        const isExpiring = dayjs(p.value).isBefore(dayjs().add(3, 'month'));
        const isExpired = dayjs(p.value).isBefore(dayjs());
        return (
          <Chip size="small" label={dayjs(p.value).format('DD MMM YY')}
            sx={{
              fontSize: 11, fontWeight: 600,
              bgcolor: isExpired ? '#FFEBEE' : isExpiring ? '#FFF8E1' : '#F5F5F5',
              color: isExpired ? '#C62828' : isExpiring ? '#F57F17' : 'text.secondary',
            }} />
        );
      },
    },
    ...
    },
    {
      ...
      renderCell: (p) => (
        <Tooltip title={dayjs(p.value).format('DD MMM YYYY, hh:mm A')}>
          <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>
            {dayjs(p.value).fromNow()}
          </Typography>
        </Tooltip>
      ),
    },
    {
      field: 'approvalStatus', headerName: 'Approval', width: 100,
      renderCell: (p) => {
        const c = APPROVAL_COLORS[p.value] || APPROVAL_COLORS['Pending'];
        return <Chip size="small" label={p.value} sx={{ fontSize: 10, fontWeight: 700, bgcolor: c.bg, color: c.color }} />;
      },
    },
    {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, height: '100%', p: 0 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.3 }}>
            Track and manage real-time stock across warehouses and batches
          </Typography>
        </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2 }}>
        ...
        />
        <DashboardCard
          title="Reserved Stock" value={stats.reserved.toLocaleString()}
          subtitle={`₹${(stats.totalValue / 100000).toFixed(1)}L total value`}
          icon={<ReservedIcon />} gradient="linear-gradient(135deg, #6A1B9A, #8E24AA)"
          bgLight="#F3E5F5" valueColor="#6A1B9A"
        />
      </Box>
      ...
      <Paper elevation={0} sx={{ borderRadius: 2, border: '1px solid #E0E0E0' }}>
        <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)}
          ...
          <Tab icon={<HistoryIcon sx={{ fontSize: 18 }} />} iconPosition="start"
            label={<Badge badgeContent={auditLog.length} color="primary" max={99} sx={{ '& .MuiBadge-badge': { fontSize: 10 } }}>Stock Movement History&nbsp;&nbsp;</Badge>} />
        </Tabs>
      </Paper>
        ...
        <>
          ...
          <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid #E0E0E0', display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <FormControl size="small" sx={{ minWidth: 160 }}>
              ...
              <Select value={warehouseFilter} label="Warehouse" onChange={e => setWarehouseFilter(e.target.value)} sx={{ borderRadius: 2 }}>
                {warehouses.map((w, idx) => <MenuItem key={`wh-${w}-${idx}`} value={w}>{w}</MenuItem>)}
              </Select>
            </FormControl>
      ...
      {activeTab === 1 && (
        <Paper elevation={0} sx={{ borderRadius: 2, border: '1px solid #E0E0E0', overflow: 'hidden' }}>
          <Box sx={{ px: 3, py: 2, bgcolor: '#F5F8FF', borderBottom: '1px solid #E0E0E0', display: 'flex', alignItems: 'center', gap: 1 }}>
            <HistoryIcon sx={{ color: BLUE.main }} />
            <Typography sx={{ fontWeight: 700, color: BLUE.dark }}>Inventory Audit Trail</Typography>
            <Chip size="small" label={`${auditLog.length} entries`} sx={{ ml: 1, fontSize: 11, fontWeight: 600, bgcolor: BLUE.bg, color: BLUE.main }} />
          </Box>
          <Box sx={{ maxHeight: 500, overflow: 'auto' }}>
            {auditLog.map((log, i) => (
              <Box key={log.id} sx={{
                ...
                ...(i % 2 === 0 && { bgcolor: '#FDFDFF' }),
              }}>
                <Avatar sx={{
                  width: 36, height: 36, fontSize: 12, fontWeight: 700,
                  bgcolor: log.action.includes('Inward') || log.action === 'Created' ? '#E8F5E9'
                    : log.action.includes('Outward') || log.action === 'Deleted' ? '#FFEBEE'
                      : log.action === 'Approval' ? '#E3F2FD' : '#FFF8E1',
                  color: log.action.includes('Inward') || log.action === 'Created' ? '#2E7D32'
                    : log.action.includes('Outward') || log.action === 'Deleted' ? '#C62828'
                      : log.action === 'Approval' ? '#1565C0' : '#F57F17',
                }}>
                  {log.action[0]}
                </Avatar>
                <Box sx={{ flex: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography sx={{ fontSize: 13, fontWeight: 700 }}>{log.action}</Typography>
                    <Chip size="small" label={log.itemCode}
                      sx={{ fontSize: 10, fontWeight: 700, fontFamily: 'monospace', bgcolor: '#F5F5F5' }} />
                  </Box>
                  <Typography sx={{ fontSize: 12, color: 'text.secondary', mt: 0.3 }}>{log.details}</Typography>
                </Box>
                <Box sx={{ textAlign: 'right', minWidth: 100 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, justifyContent: 'flex-end' }}>
                    <Typography sx={{ fontSize: 12, color: '#C62828', fontWeight: 600, textDecoration: 'line-through' }}>{log.oldValue}</Typography>
                    <Typography sx={{ fontSize: 11, color: 'text.disabled' }}>→</Typography>
                    <Typography sx={{ fontSize: 12, color: '#2E7D32', fontWeight: 700 }}>{log.newValue}</Typography>
                  </Box>
                ...
                <Chip size="small" label={log.user}
                  sx={{ fontSize: 10, fontWeight: 600, bgcolor: '#EDE7F6', color: '#4527A0', minWidth: 60 }} />
              </Box>
            ))}
          </Box>
        </Paper>
...
const DashboardCard = ({ title, value, subtitle, icon, gradient, bgLight, valueColor }) => (
