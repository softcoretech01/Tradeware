...
import { toggleWarehouseStatus, addWarehouse, updateWarehouse, deleteWarehouse } from '../../store/locationsSlice';
...
const LocationMaster = () => {
  ...
  const filteredWarehouses = warehouses.filter(w => {
    const matchesName = appliedSearchName 
      ? w.name.toLowerCase().includes(appliedSearchName.toLowerCase())
      : true;

    const matchesKeyword = appliedKeyword 
      ? (
          w.name.toLowerCase().includes(appliedKeyword.toLowerCase()) ||
          w.id.toLowerCase().includes(appliedKeyword.toLowerCase()) ||
          w.address.toLowerCase().includes(appliedKeyword.toLowerCase()) ||
          (w.manager && w.manager.toLowerCase().includes(appliedKeyword.toLowerCase()))
        )
      : true;
  ...
  return (
    <div className="page-container fade-in">
      ...
      <div className="card filter-card">
        <div className="filter-row">
          <div className="filter-group">
            <label>Warehouse Name / Location</label>
            <input 
              type="text" 
      ...
      <div className="card table-card">
          ...
          <table>
            <thead>
              <tr>
                ...
                <th><span className="th-content">Warehouse Name <Filter size={12} /></span></th>
                <th><span className="th-content">Warehouse Address <Filter size={12} /></span></th>
                <th>Active</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              {paginatedWarehouses.length > 0 ? (
                paginatedWarehouses.map(warehouse => (
                  <tr key={warehouse.id}>
                    <td className="code-cell">{warehouse.id}</td>
                    <td className="bold-cell">{warehouse.name}</td>
                    <td className="address-cell">{warehouse.address}</td>
                    <td>
                      <label className="switch">
                        <input 
                          type="checkbox" 
                          checked={warehouse.active} 
                          onChange={() => dispatch(toggleWarehouseStatus(warehouse.id))}
                        />
                    ...
                    <td>
                      <div className="action-buttons">
                        <button className="edit-btn" onClick={() => handleEdit(warehouse)} title="Edit"><Edit size={19} /></button>
                        <button className="delete-btn" onClick={() => handleDelete(warehouse.id)} title="Delete"><Trash2 size={19} /></button>
                      </div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan="5" className="empty-row-cell">
                    No warehouses found matching the search criteria.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {/* Pagination (Conditionally hidden for few data entries) */}
        {filteredWarehouses.length > pageSize && (
          <div className="pagination">
            <div className="pagination-info">
              Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, filteredWarehouses.length)} of {filteredWarehouses.length} entries
            </div>
