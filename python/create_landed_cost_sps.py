...
try:
    with engine.connect() as con:
        # SP_GetImportLandedCostHeader
        con.execute(text("DROP PROCEDURE IF EXISTS SP_GetImportLandedCostHeader"))
        sp_get_header = text("""
        CREATE PROCEDURE SP_GetImportLandedCostHeader(IN p_import_po_id INT)
        BEGIN
            SELECT * FROM Purchase_Masters.import_landed_costs_Header
            WHERE import_po_id = p_import_po_id;
        END
        """)
        con.execute(sp_get_header)

        # SP_GetImportLandedCostDetails
        con.execute(text("DROP PROCEDURE IF EXISTS SP_GetImportLandedCostDetails"))
        sp_get_details = text("""
        CREATE PROCEDURE SP_GetImportLandedCostDetails(IN p_import_landed_cost_id INT)
        BEGIN
            SELECT * FROM Purchase_Masters.import_landed_costs_Details
            WHERE import_landed_cost_id = p_import_landed_cost_id;
        END
        """)
        con.execute(sp_get_details)

        # SP_SaveImportLandedCostHeader
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO Purchase_Masters.import_landed_costs_Header (
                    ...
                    total_customs_duty, total_freight, total_port_charges, total_overhead, total_landed_cost
                ) VALUES (
                    ...
                    p_total_customs_duty, p_total_freight, p_total_port_charges, p_total_overhead, p_total_landed_cost
                );
        ...
        # SP_SaveImportLandedCostDetail
        con.execute(text("DROP PROCEDURE IF EXISTS SP_SaveImportLandedCostDetail"))
        sp_save_detail = text("""
        CREATE PROCEDURE SP_SaveImportLandedCostDetail(
            IN p_import_landed_cost_id INT,
            IN p_item_id VARCHAR(50),
            IN p_qty DECIMAL(15,4),
            IN p_fob_val_lcy DECIMAL(15,4),
            IN p_allocated_overhead DECIMAL(15,4),
            IN p_total_landed_cost DECIMAL(15,4),
            IN p_landed_unit_cost DECIMAL(15,4)
        )
        BEGIN
            INSERT INTO Purchase_Masters.import_landed_costs_Details (
                import_landed_cost_id, item_id, qty, fob_val_lcy, allocated_overhead, total_landed_cost, landed_unit_cost
            ) VALUES (
                p_import_landed_cost_id, p_item_id, p_qty, p_fob_val_lcy, p_allocated_overhead, p_total_landed_cost, p_landed_unit_cost
            );
        END
        """)
        con.execute(sp_save_detail)
