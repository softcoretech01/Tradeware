...
jsx_code = """import { formatDate } from '../../utils/dateUtils';
...
const ImportPurchase = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [posRes, supRes, itemRes, ptRes] = await Promise.all([
...
with open(r"d:\Trade Wave\react\src\pages\ImportManagement\ImportPurchase.jsx", "w", encoding="utf-8") as f:
    f.write(jsx_code)

print("ImportPurchase.jsx rewritten.")
